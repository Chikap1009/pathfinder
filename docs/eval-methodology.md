...
The two strata together tell the story. On the **lexically-anchored
original** queries BM25 / RRF dominate because skill-name tokens overlap
doc text directly. On the **natural-language paraphrase** queries
**only the cross-encoder rerank lifts metrics** (+9.4 % nDCG@10 and
+32.4 % Recall@10 over RRF) because bge-reranker-v2-m3 is trained on
natural-language pairs (MS-MARCO, MIRACL).

This is the textbook hybrid-retrieval architecture playing out: each stage
handles a different query distribution. No single signal is robust across
both; the multi-stage funnel is.
